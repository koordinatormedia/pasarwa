...
execPHP.phpFolder = 'C:\\xampp\\htdocs\\wablast\\';
...
cron.schedule('10 * * * *',  async function() {
 ...
 // console.log('ada init')
  const jadwal= await db.cronjadwal();
      
});

/* client.on('ready',()=>{
	client.isRegisteredUser(number).then(c=> console.log(c)).catch(err=>console.log(err))
	client.getNumberId(number).then(c=>console.log(c)).catch(err=>console.log(err))
	console.log('Client is ready!');
}); */

client.on('message', async message => {
	//console.log('Id Pesan => '+message.mediaKey.toString());
	const pengirim = message.from;
	const inbox = message.body;
	//const masukdb= await db.insertpesan(pengirim,inbox);
